...
function loadEntries(ddata, ctx, callback) {
    ...
    // Load 7 days so the main view can scroll back a full week (was 2 days).
    const longLoad = Math.round(constants.SEVEN_DAYS);
...
function loadTreatments(ddata, ctx, callback) {
    ...
    const withFrame = ddata.page && ddata.page.frame;
    // Load 7.5 days to cover the 7-day main-view window including overlapping
    // temp boluses or temp targets for first load (was 2.5 days).
    const longLoad = Math.round(constants.SEVEN_DAYS + (constants.ONE_DAY / 2));
