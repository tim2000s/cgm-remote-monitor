...
agp.report = function report_agp (datastorage, sorteddaystoshow, options) {
  ...
  bands.forEach(function (band) {
    tirHtml += '<div class="agp-tir-row' + (band.target ? ' agp-target' : '') + '">' +
      ...
      '<span class="agp-tir-time">' + band.hh + 'h ' + band.mm + 'min/' + translate('day') + ' ' + translate('avg') + '</span>' +
      '</div>';
  });
  tirHtml += '</div>';
  ...
  function drawAGP () {
    ...
    var targetLow = options.targetLow;   // display units
    var targetHigh = options.targetHigh;  // display units
