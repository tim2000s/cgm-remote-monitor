...
glucosedistribution.html = function html (client) {
  ...
  var ret =
    ...
    '<div class="gd-cards">' +
    '<div class="gd-card"><div id="glucosedistribution-report"></div></div>' +
    '<div class="gd-card"><div id="glucosedistribution-stability"></div></div>' +
    '</div>' +
    '<div class="gd-hours">' +
...
glucosedistribution.css =
  ...
  '.gd-rangeselect { margin-bottom: 14px; }' +
  '.gd-summary { margin: 14px 0; }' +
  '#glucosedistribution-overviewchart { width: 100%; margin: 6px 0 14px 0; }' +
  '.gd-tir-bar {' +
  '  display: flex;' +
  '  width: 100%;' +
  '  height: 28px;' +
  ...
  '  print-color-adjust: exact;' +
  '}' +
  '.gd-tir-legend-pct { font-weight: bold; margin-left: 6px; }' +
  '.gd-cards {' +
  '  display: flex;' +
  '  flex-wrap: wrap;' +
  '  gap: 16px;' +
...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  // Stacked Time-in-Range bar (replaces the former Flot pie chart)
  var overview = $('#glucosedistribution-overviewchart');
  overview.empty();

  var bar = $('<div class="gd-tir-bar">');
  var legend = $('<div class="gd-tir-legend">');
  ...
  bands.forEach(function(band) {
    ...
    var row = $('<div class="gd-tir-legend-row">');
    var swatch = $('<span class="gd-tir-swatch">');
    swatch.css('background-color', band.color);
    row.append(swatch);
    row.append(document.createTextNode(label));
    var pctSpan = $('<span class="gd-tir-legend-pct">');
    pctSpan.text(pctText);
    row.append(pctSpan);
    legend.append(row);
  });
