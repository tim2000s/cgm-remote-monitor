...
glucosedistribution.html = function html (client) {
  ...
  var ret =
    ...
    ')' +
    '  </h2>' +
    '<div id="glucosedistribution-rangeselect" style="margin-bottom: 10px;">' +
    ...
    '<label for="glucosedistribution-rangemode-tir">' + translate('Standard (Time in Range)') + '</label> ' +
    '<input type="radio" name="glucosedistribution-rangemode" id="glucosedistribution-rangemode-tight" value="tight">' +
    '<label for="glucosedistribution-rangemode-tight">' + translate('Tight Range') + '</label> ' +
    ...
    '</div>' +
    '<div id="glucosedistribution-summary" style="margin-bottom: 10px;"></div>' +
    '<table><tr>' +
...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  if (rangeMode === 'custom') {
    bands = [
      { key: 'Low', label: 'Low', exp: ' (<' + options.targetLow + ')', color: '#f88', filter: function(r) { return r.sgv > 0 && r.sgv < options.targetLow; } }
      , { key: 'Normal', label: 'In Range', exp: '', color: '#8f8', filter: function(r) { return r.sgv >= options.targetLow && r.sgv < options.targetHigh; } }
      , { key: 'High', label: 'High', exp: ' (>=' + options.targetHigh + ')', color: '#ff8', filter: function(r) { return r.sgv >= options.targetHigh; } }
    ];
  } else if (rangeMode === 'tight') {
    bands = [
      { key: 'VeryLow', label: 'Very Low', exp: ' (<' + bgNum(54) + ')', color: '#b71c1c', filter: function(r) { return r.bgValue < 54; } }
      , { key: 'Low', label: 'Low', exp: ' (' + bgNum(54) + '–' + bgNum(69) + ')', color: '#f88', filter: function(r) { return r.bgValue >= 54 && r.bgValue < 70; } }
      , { key: 'InTightRange', label: 'In Tight Range', exp: ' (' + bgNum(70) + '–' + bgNum(140) + ')', color: '#8f8', filter: function(r) { return r.bgValue >= 70 && r.bgValue <= 140; } }
      , { key: 'InRange', label: 'In Range', exp: ' (' + bgNum(141) + '–' + bgNum(180) + ')', color: '#cfc', filter: function(r) { return r.bgValue > 140 && r.bgValue <= 180; } }
      , { key: 'High', label: 'High', exp: ' (' + bgNum(181) + '–' + bgNum(250) + ')', color: '#ff8', filter: function(r) { return r.bgValue > 180 && r.bgValue <= 250; } }
    ...
    bands = [
      { key: 'VeryLow', label: 'Very Low', exp: ' (<' + bgNum(54) + ')', color: '#b71c1c', filter: function(r) { return r.bgValue < 54; } }
      , { key: 'Low', label: 'Low', exp: ' (' + bgNum(54) + '–' + bgNum(69) + ')', color: '#f88', filter: function(r) { return r.bgValue >= 54 && r.bgValue < 70; } }
      , { key: 'InRange', label: 'In Range', exp: ' (' + bgNum(70) + '–' + bgNum(180) + ')', color: '#8f8', filter: function(r) { return r.bgValue >= 70 && r.bgValue <= 180; } }
      , { key: 'High', label: 'High', exp: ' (' + bgNum(181) + '–' + bgNum(250) + ')', color: '#ff8', filter: function(r) { return r.bgValue > 180 && r.bgValue <= 250; } }
      , { key: 'VeryHigh', label: 'Very High', exp: ' (>' + bgNum(250) + ')', color: '#ffb74d', filter: function(r) { return r.bgValue > 250; } }
    ];
  }

  var colors = bands.map(function(band) {
    return band.color;
  });
