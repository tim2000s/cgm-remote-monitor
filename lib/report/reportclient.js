...
var init = function init () {
  ...
  client.init(function loaded () {
    ...
    // Auto-generate the default (7-day) report on load when launched in a real
    // browser (reportinit.js sets this flag). The date range already defaults
    // to the last 7 days with the date filter enabled, so this renders the
    // Statistics Summary immediately, without a manual "Show" click.
    //
    // show() needs profile/timezone data, which only becomes available on
    // client.sbx.data after the first data-update cycle. Poll until it is
    // ready, capped at ~10s so we never spin forever (and so the test harness,
    // which runs setTimeout synchronously, gives up after a bounded number of
    // iterations instead of looping).
    if (Nightscout.reportAutoShow) {
      ...
      (function waitForProfileThenShow () {
        ...
        } else if (autoShowAttempts < 40) {
          window.setTimeout(waitForProfileThenShow, 250);
        } else {
          console.warn('Auto-show: report data not ready in time, skipping initial render');
        }
      })();
    }
