...
function init (client, $) {
  ...
  widget.refresh = function refresh () {
    ...
    $.ajax({
      ...
      // e.g. 401 on unauthorized views - hide and try again on the next cycle
      console.error('TIR widget: failed to load entries, hiding until next refresh');
  ...
  widget.load = function load () {
    ...
    started = true;
    widget.refresh();
    window.setInterval(widget.refresh, REFRESH_INTERVAL_MS);
  };
