...
function init (client, d3, $) {
  ...
  // drag-to-pan the focus (main) chart horizontally by moving the context brush.
  // Reuses the existing brush -> brushed -> scroll render pipeline, so no new
  // rendering path. Works for mouse and touch (d3.drag handles both).
  function panToPan () {
    return d3.drag()
      .on('start', function panStart () {
        chart.panSurface.style('cursor', 'grabbing');
      })
      .on('drag', function panned (event) {
        if (!chart.theBrush) { return; }

        var domain = chart.xScale.domain();
        var range = chart.xScale.range();
        var pxSpan = range[1] - range[0];
        if (!pxSpan) { return; }

        // convert the pixel delta into a time delta; drag right reveals
        // earlier data (grab-and-pull), so invert the sign
        var msPerPx = (domain[1].getTime() - domain[0].getTime()) / pxSpan;
        var dt = -event.dx * msPerPx;

        var brushExtent = chart.createBrushedRange();
        var windowMS = brushExtent[1].getTime() - brushExtent[0].getTime();

        var bounds = client.dataExtent();
        var minStart = bounds[0].getTime();
        var maxEnd = chart.inRetroMode() ? bounds[1].getTime() : Math.max(client.now, bounds[1].getTime());

        var start = brushExtent[0].getTime() + dt;
        var end = start + windowMS;

        // clamp within the loaded data window, preserving the window width
        if (start < minStart) { start = minStart; end = start + windowMS; }
        if (end > maxEnd) { end = maxEnd; start = end - windowMS; }

        chart.theBrush.call(chart.brush.move, [start, end].map(function toPixels (t) {
          return chart.xScale2(new Date(t));
        }));
      })
      .on('end', function panEnd () {
        chart.panSurface.style('cursor', 'grab');
      });
  }
  ...
  // transparent surface to capture drag-to-pan gestures on the main chart.
  // Appended first so it sits behind the dots, threshold lines and axes
  // (so tooltips and treatment-dot dragging keep working), but still captures
  // pointer events over empty plot area for both mouse and touch.
  chart.panSurface = chart.focus.append('rect')
  ...
  chart.update = function update (init) {
    ...
    if (widthChanged || (chart.prevChartHeight !== chartHeight)) {
      ...
      // size the drag-to-pan capture surface to the focus plot area
      chart.panSurface
