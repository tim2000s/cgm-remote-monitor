...
function init (client, d3) {
  ...
  renderer.addFocusCircles = function addFocusCircles () {
    ...
    // Only render the dots within the currently visible focus window (plus a
    // small margin so edges look seamless while panning). This keeps the focus
    // redraw cost bounded by the window width rather than the full loaded
    // history, so longer histories don't make scrolling janky.
    var focusDomain = chart().xScale.domain();
    var focusMarginMS = (focusDomain[1].getTime() - focusDomain[0].getTime()) * 0.15;
    var focusLo = focusDomain[0].getTime() - focusMarginMS;
    var focusHi = focusDomain[1].getTime() + focusMarginMS;
    var focusData = client.entries.filter(function inFocusWindow (d) {
      return d.mills >= focusLo && d.mills <= focusHi;
    });
