...
var SEVEN_DAYS = 604800000; // keep up to 7 days client-side for the main-view scrollback
...
function receiveDData (received, ddata, settings) {
  ...
  ddata.sgvs = mergeDataUpdate(received.delta, ddata.sgvs, received.sgvs, SEVEN_DAYS);
  ddata.mbgs = mergeDataUpdate(received.delta, ddata.mbgs, received.mbgs, SEVEN_DAYS);
  ...
  if (received.devicestatus) {
    if (settings.extendedSettings.devicestatus && settings.extendedSettings.devicestatus.advanced) {
      ...
      ddata.devicestatus = mergeDataUpdate(received.delta, ddata.devicestatus, received.devicestatus, SEVEN_DAYS);
    } else {
